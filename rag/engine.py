...
from datetime import date  # date de consultation des sources web
...
import io  # permet de personnaliser le chargement pickle
import json  # lecture stricte des reponses JSON du reranker LLM
import math  # calcul du score BM25
...
import unicodedata  # normalisation des accents pour la recherche lexicale
from pathlib import Path  # gestion propre des chemins fichiers
from urllib.parse import urlparse  # analyse des URLs web
...
def charger_env_local() -> None:
    ...
    Pourquoi ne pas importer `python-dotenv` au demarrage du module ?
    Dans cet environnement local, l'import de `dotenv` peut prendre plusieurs
    secondes. Streamlit réimporte souvent l'application ; on garde donc le
    demarrage leger et on charge `.env` seulement dans les chemins qui en ont
    besoin : indexation, recherche CLI et pipeline complet.
    """
...
class UnpicklerCompatibleRag(pickle.Unpickler):
    ...
    Cette classe dit simplement :
    - si tu vois `__main__.Morceau`, utilise `rag_pdf.Morceau`.
    """
...
def extraire_tableaux_page(page) -> list[str]:
    ...
    : watts, capacites, SLA, dimensions. Si on les laisse seulement dans le texte
    brut du PDF, l'ordre des colonnes peut etre degrade. Un chunk tableau separe
    donne au retrieval et au modele une structure beaucoup plus claire.
    """
...
def extraire_extraits_techniques(texte: str, numero_page: int) -> list[str]:
    ...
    Ces extraits techniques donnent au retrieval des passages plus concentres
    que le bloc de page complet.
    """
...
# -> On respecte la structure naturelle du document.
# -> On peut citer une source precise : nom du PDF + numero de page.
...
def extraire_sections_simples(texte: str) -> list[tuple[str, str]]:
    ...
    inferer les titres a partir de signaux simples : ligne courte, isolée, peu de
...
def ligne_parasite_pdf(ligne: str) -> bool:
    ...
    Ces lignes ne portent pas de contenu métier. Si on les indexe comme titres,
...
# Pourquoi decouper ?
# -> Un modele ne doit pas recevoir un document entier a chaque question.
# -> Le RAG cherche seulement les passages utiles.
#
# Pourquoi un chevauchement ?
# -> Pour eviter de couper une idee importante en deux.
# -> Cela ameliore souvent la qualite de la recherche.
...
def indexer_pdf() -> None:
    """
    Cree l'index vectoriel FAISS a partir des PDF.
    ...
    # Comme les vecteurs sont normalises, cela revient a chercher par
    # similarite cosinus.
    index = faiss.IndexFlatIP(dimension)
    index.add(vecteurs)

    # Sauvegarde atomique de l'index FAISS.
    #
    # Principe :
    # - on ecrit d'abord dans des fichiers temporaires ;
    # - puis on remplace les anciens fichiers d'un seul coup avec os.replace().
    #
    # Cela evite qu'une question Streamlit lise index.faiss exactement pendant
    # qu'une reindexation est en train de l'ecrire.
    ...
    # Sauvegarde des textes et des sources.
    # FAISS stocke les vecteurs, mais pas le texte original.
...
# Pourquoi FAISS ?
# -> C'est une base vectorielle locale et rapide.
# -> Elle permet de chercher dans beaucoup de vecteurs.
#
# Pourquoi sauvegarder aussi morceaux.pkl ?
# -> FAISS retourne des numeros de resultats.
# -> morceaux.pkl permet de retrouver le texte, le fichier et la page.
...
def scores_bm25(question: str, textes: list[str]) -> list[float]:
    ...
    for document, longueur_document in zip(documents, longueurs):
        ...
        for mot in mots_question:
            ...
            normalisation_longueur = 1 - b + b * (longueur_document / max(1, longueur_moyenne))
            score += idf * (frequence * (k1 + 1)) / (frequence + k1 * normalisation_longueur)
